<template>
  <div class="bg-gray-50 dark:bg-gray-800  min-h-screen p-4">
    <!-- Grid-Container -->
    <div
        class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 place-items-center"
    >
      <!-- Einzelnes Buch -->
      <div
          v-for="book in books"
          :key="book.isbn"
          class="flex flex-col items-center bg-white  dark:bg-gray-800
               shadow-lg rounded-lg p-4 w-64 transition-shadow hover:shadow-xl"
      >
        <!-- Buch-Cover -->
        <img
